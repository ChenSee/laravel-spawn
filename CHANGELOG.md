...
- **A session handler registered at boot was unsupported under async serving.** `Session::extend('mongo', ...)` writes into the manager, which each coroutine rebuilt, giving `Driver [mongo] not supported` on the first request. The manager now adopts boot-time handlers the way the auth manager adopts drivers.
